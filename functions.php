...
function rehub_framework_register_scripts() {
	...
	// Stylesheets
	wp_register_style('rhstyle', get_stylesheet_directory_uri() . '/style.css', array(), '6.9.8');
	wp_register_style('responsive', get_template_directory_uri() . '/css/responsive.css', array(), '6.9.8');
	...
	wp_register_style( 'rehub-woocommerce', get_template_directory_uri().'/css/woocommerce.css', array(), '6.9.8', 'all' );
	...
	wp_register_style('eggrehub', get_template_directory_uri() . '/css/eggrehub.css', array(), '6.9.8');
	...
	wp_register_style('rhwcvendor', get_template_directory_uri() . '/css/wcvendor.css', array(), '6.9.3');
	
	//Scripts
	wp_register_script('rehub', get_template_directory_uri() . '/js/custom.js', array('jquery'), '6.9', true);
	wp_register_script('flexslider', get_template_directory_uri() . '/js/jquery.flexslider-min.js', array('jquery'), '2.2.2', true);
	wp_register_script('prettyphoto', get_template_directory_uri() . '/js/jquery.prettyPhoto.js', array('jquery'), '3.1.7', true);
	...
	wp_register_script( 'custom_pretty', get_template_directory_uri() . '/shortcodes/js/custom_pretty.js', array('jquery'), '1.0', true );
...
add_filter('widget_text_content', 'wpautop', 100);
//add_filter('widget_text', 'do_shortcode', 1000);
add_filter('widget_text_content', 'do_shortcode', 1000);
		...
		if( $ubermenu == false ) {
			...
			if($args->theme_location == 'primary-menu' && rehub_option('rehub_login_icon') == 'menu' && rehub_option('userlogin_enable') == '1') {
				$items .= '<li class="menu-item rehub-custom-menu-item rehub-top-login-onclick menu-item-one-line">';
					$items .= wpsm_user_modal_shortcode(array('wrap'=> 'a'));
				$items .= '</li>';				
			}			
...
function wpsm_thumb( $size = 'small', $lazy = true ){
	if( $size == 'medium_news' ){$width = 406; $height = 220; $nothumb = get_template_directory_uri() . '/images/default/noimage_370_220.png' ;}
	elseif( $size == 'med_thumbs' ){$width = 123; $height = 90; $nothumb = get_template_directory_uri() . '/images/default/noimage_123_90.png' ;}	
	elseif( $size == 'feature_slider' ){$width = 788; $height = 460; $nothumb = get_template_directory_uri() . '/images/default/noimage_765_460.jpg' ;}
	elseif( $size == 'post-thumbnail' ){$width = 788; $height = 460; $nothumb = get_template_directory_uri() . '/images/default/noimage_765_460.jpg' ;}
				...
				<div class="comm_meta_wrap">
					<?php if (rehub_option('rh_enable_mycred_comment') == 1):?>
						<?php 
							...
							if(function_exists('mycred_display_users_total_balance') && $author_id !=0){
								if(rehub_option('rh_mycred_custom_points')){
									$custompoint = rehub_option('rh_mycred_custom_points');
									$mycredpoint = mycred_display_users_total_balance($author_id, $custompoint );
								}
								else{
									$mycredpoint = mycred_display_users_total_balance($author_id);			
								}
							}							
...
function rehub_admin_error_notice() {
    ...
    if ( ! get_user_meta($user_id, 'ignore_notices_rehub68') ) {
		if ($page=='rehub' || $page=='rehub-support' || $page=='rehub-plugins' || $page=='rehub-demos' || $page=='vpt_option' ) {
			$class = "error";
			$message = 'Rehub 6.9 is major update for Woocommerce 3.0 version, Dokan support. Also Revolution Slider was added to bundle. Due to many changes, you can have some issues. Please, update cache on site after updating theme' ;
...
function rehub_nag_ignore() {
    ...
    if ( isset($_GET['rehub_nag_ignore']) && '0' == $_GET['rehub_nag_ignore'] ) {
        add_user_meta($user_id, 'ignore_notices_rehub69', 'true', true);
